  ...
  ($q, $cookies) ->
    (gistId) ->
      github = new Github
        ...
        auth: 'oauth'
        apiUrl: if !!window.SassMeister.isEmbedded then '{%& embed_domain %}/app' else null
...
.factory '$sassMeisterGist', [
  ...
  ($q, $cookies, $githubGist) ->
    create: (scope, cb) ->
      ...
      if scope.preferences.autoprefixer and scope.preferences.autoprefixerBrowsers
        files['browserslist'] =
          content: scope.preferences.autoprefixerBrowsers.split(', ').join("\n")
    ...
    update: (id, scope, cb) ->
      ...
      if scope.gist.files.browserslist and not scope.preferences.autoprefixer
        files['browserslist'] = null
